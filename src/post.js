...
function buildImageUrl(pillar) {
  const prompts = {
    PAIN: 'dark crimson red background with dramatic gold geometric patterns, luxury abstract art, no text, no letters, no words, professional social media background, moody cinematic lighting, 4k',
    PROOF: 'deep dark red background with gold glowing lines and abstract data visualization patterns, no text, no letters, no words, premium minimalist design, dramatic lighting, 4k',
    FOMO: 'dark red gradient background with gold glowing edges and floating abstract shapes, no text, no letters, no words, urgent dramatic mood, cinematic, 4k',
    EDUCATION: 'rich burgundy dark red background with gold grid lines and clean geometric shapes, no text, no letters, no words, modern tech aesthetic, professional, 4k'
  };
 
  const prompt = encodeURIComponent(prompts[pillar] || prompts.PAIN);
...
async function postToInstagram(content) {
  ...
  const imageUrl = buildImageUrl(pillar);
